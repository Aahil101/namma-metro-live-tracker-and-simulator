...
| **Time travel** | play / pause and 1× · 1.5× · 2× · 4× playback across the whole service day |
...
tools/
  ...
  browser-check.mjs      134 end-to-end assertions in real Chrome over CDP
...
- The password is **never** in this repository or in any file the browser downloads. It
  exists only as a PBKDF2-SHA256 derivation in a Cloudflare secret.
...
- Without the API deployed, `/admin.html` runs in **local mode**: it says plainly that
  there are no server-side stats, leaves the traffic tiles blank rather than inventing
  numbers, and shows only feedback queued in that browser.

`tools/browser-check.mjs` greps the entire client bundle on every run to assert the
password and hash never appear in it.
...
267 assertions, no test framework, nothing to install.
...
node tools/browser-check.mjs   # 134  end-to-end in real headless Chrome
